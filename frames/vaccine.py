...
from constants import VACCINE_GUIDELINES
...
class VaccineFrame(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        opts = [f"{v['tr_name']} ({v['category_tr']})" for v in VACCINE_GUIDELINES.values()
    ...
    def _add(self):
        ...
        if not g: return
        applied = self.dp.get()
        nd = calc_next(applied, g["adult_interval_days"]) if g["adult_interval_days"] > 0 else ""
