...
from utils import _title, _lbl, _card, calc_age, days_remaining, remaining_badge
try:
...
class DashboardFrame(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # Başlık kartı
        hdr = _card(sc); hdr.pack(fill="x", padx=20, pady=(18,10))
        ...
        if HAS_PIL and photo and os.path.exists(photo):
            ...
            self._img = ctk.CTkImage(light_image=img, dark_image=img, size=(80,80))
            ctk.CTkLabel(hi, image=self._img, text="").pack(side="left", padx=(0,16))
        else:
            ctk.CTkLabel(hi, text="🐱", font=ctk.CTkFont(size=50)).pack(side="left", padx=(0,16))
        info = ctk.CTkFrame(hi, fg_color="transparent"); info.pack(side="left", fill="x", expand=True)
        ctk.CTkLabel(info, text=self.cname, font=ctk.CTkFont(size=24, weight="bold"),
        ...
        sub = f"{gender}  •  " + breed + f"  •  {age}" if gender else f"{breed}  •  {age}"
        ctk.CTkLabel(info, text=sub, font=ctk.CTkFont(size=13), text_color=T.TEXT_SECONDARY).pack(anchor="w")
        ...
        if urgent:
            banner = ctk.CTkFrame(sc, fg_color=T.ERROR, corner_radius=10)
            banner.pack(fill="x", padx=20, pady=(0,8))
            _lbl(banner, f"⚠️  {len(urgent)} acil takvim var!", size=14, bold=True, color="white").pack(
        ...
        # Bölümler
        self._section(sc, "💉 Yaklaşan Aşılar", self._items(db.get_vaccines, 7))
        self._section(sc, "🐛 Parazit Takvimi", self._items(db.get_parasites, 5))
    ...
    def _section(self, sc, title, items):
        _title(sc, text=title).pack(anchor="w", padx=24, pady=(14,4))
        if not items:
            _lbl(sc, "Kayıt yok.", color=T.TEXT_MUTED).pack(padx=24, pady=4); return
        for label, due, rem in items[:5]:
            bt, bc = remaining_badge(rem)
            row = _card(sc); row.pack(fill="x", padx=24, pady=2)
            _lbl(row, label, size=13, bold=True, color=T.TEXT_PRIMARY).pack(side="left", padx=12, pady=10)
    ...
    def _weight_summary(self, sc):
        _title(sc, text="⚖️ Son Kilo").pack(anchor="w", padx=24, pady=(14,4))
        ws = db.get_weights(self.cid)
        if not ws: _lbl(sc, "Kayıt yok.", color=T.TEXT_MUTED).pack(padx=24, pady=4); return
        w = ws[0]; row = _card(sc); row.pack(fill="x", padx=24, pady=2)
        _lbl(row, f"{w[3]} kg", size=16, bold=True, color=T.TEXT_PRIMARY).pack(side="left", padx=12, pady=10)
    ...
    def _appointments(self, sc):
        ...
        if not apps: _lbl(sc, "Planlanmış randevu yok.", color=T.TEXT_MUTED).pack(padx=24, pady=4); return
        for a in sorted(apps, key=lambda x: x[2])[:3]:
            row = _card(sc); row.pack(fill="x", padx=24, pady=2)
            rem = days_remaining(a[2]); bt, bc = remaining_badge(rem)
            _lbl(row, f"{a[2]} {a[3]}", size=13, bold=True, color=T.TEXT_PRIMARY).pack(side="left", padx=12, pady=10)
    ...
    def _expense_summary(self, sc):
        ...
        if not es: _lbl(sc, "Kayıt yok.", color=T.TEXT_MUTED).pack(padx=24, pady=4); return
        total = sum(e[4] for e in es)
        row = _card(sc); row.pack(fill="x", padx=24, pady=2)
        _lbl(row, f"Toplam: {total:,.2f} ₺", size=16, bold=True, color=T.TEXT_PRIMARY).pack(side="left", padx=12, pady=10)
